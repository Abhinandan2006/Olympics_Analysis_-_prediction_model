...
df = preprocessor.preprocess_winter(athlete_data, region_data)
st.sidebar.markdown("<h1 style='text-align: center;'>winter Olympics Analysis</h1>", unsafe_allow_html=True)
image_path = os.path.join(os.path.dirname(__file__), 'image.png')
if os.path.exists(image_path):
    image = Image.open(image_path)
    st.sidebar.image(image, caption='Winter Olympics')
...
if user_menu == "Country-Wise Analysis":
    ...
    if pt.empty:
        st.warning("Data not available for " + selected_country + " please select another country")
    else:
        fig = px.line(
        country_df,
        x="Year",
...
if user_menu == 'Athlete-Wise Analysis':
    ...
    if x:  # only plot if we have valid data
        ...
        st.markdown(f"<h1 style='text-align: center;'>Distribution of Age with resepect to Sports (Gold Medalists)</h1>", unsafe_allow_html=True)
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("Not enough valid data to display age distribution.")


        st.markdown(f"<h1 style='text-align: center;'>Distribution of Age wrt Sports (Gold Medalists)</h1>", unsafe_allow_html=True)
